...
export function AgeOptions({ setTotalAdults, setTotalKids, setIsOpen }: ageOptionsProps) {
	...
	const [basket, setBasket] = useSessionStorage<BasketType>('basket', initialStates.basket);
	...
	function getTotalKids() {
		return options.filter(({ id }) => id !== 'A').reduce((sum, item) => sum + item.count, 0);
	}
	...
	return (
		<div>
			...
			<Button className="mt-4 w-full font-bold" variant="destructive" onClick={handleApply}>
				Apply
			</Button>
		</div>
	);
